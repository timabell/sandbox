#!/usr/bin/env bash
# NixOS variant of sandbox.sh.
# On NixOS the real binaries live in /nix/store and are exposed via
# /run/current-system/sw/bin; /usr/bin/bash does not exist. This script
# binds those paths instead of overlaying /usr/bin/bash.
...
args=(
  ...
  --setenv PATH "/run/current-system/sw/bin:/usr/local/bin:/usr/bin:/bin"  # minimal PATH; mise activate extends it
)
